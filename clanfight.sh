...
# /view
	echo ""
	( exec $PAGE $URL/clanfight -o user_agent="$(shuf -n1 .ua)" | head -n15 | tail -n14 | sed "/\[user\]/d;/\[arrow\]/d;/\ \[/d" | grep --color $ACC ) &
	_timexit 10
	_unset
	echo "ClanFight(✔)"
	sleep 30
...
_clanfight () {
	case $(date +%H:%M) in
		(10:5[5-9]|18:5[5-9])
			...
			_timexit 10
			( exec $SOURCE "$URL/clanfight/?close=reward" -o user_agent="$(shuf -n1 .ua)" >$TMP/SRC ) &
			_timexit 10
			( exec $SOURCE "$URL/clanfight/enterFight" -o user_agent="$(shuf -n1 .ua)" >$TMP/SRC ) &
			_timexit 10
			...
			echo -e "\nClan fight"
			echo $URL
			ACCESS=$(sed -n 1p <<<$(grep -o -P '(/clanfight(/\w+/\Wr\W\d+|/))' $TMP/SRC) 2> /dev/null)
			echo -e " 👣 Entering...\n$ACCESS"
# /wait
			echo " 😴 Waiting..."
			until [[ $(echo $ACCESS | grep -o 'clanfight/dodge/') ]] ; do
				[[ $(date +%M) = 00 && $(date +%S) > 19 ]] && break
				echo -e " 💤	...\n$ACCESS"
				( exec $SOURCE "$URL/clanfight/" -o user_agent="$(shuf -n1 .ua)" >$TMP/SRC ) &
				_timexit 10
				ACCESS=$(sed -n 1p <<<$(grep -o -P '(/clanfight(/\w+/\Wr\W\d+|/))' $TMP/SRC) 2> /dev/null)
				sleep 3
			done
